"use strict";
const options = {
    moreFilterCls: '.more-filter-cls',
    hideMoreFilterCls: '.hide-more-filter-cls',
...
var BlsEventCollectionShopify = (function() {
    return {
        ...
        setupEventListeners: function() {
            ...
            // puvii added on 13th november
            document.querySelectorAll(options.moreFilterCls).forEach(btn => {
                btn.addEventListener("click", event => {
                    event.preventDefault();

                    if (document.querySelector(options.moreFilterCls).classList.contains('more-filter-show')) {
                        document.querySelector(options.moreFilterCls).classList.replace('more-filter-show', 'more-filter-hide');
                        document.querySelector(options.hideMoreFilterCls).classList.replace('more-filter-hide', 'more-filter-show');

                        document.querySelectorAll('.custom-desktop-filter-item').forEach(btn => {
                            if(btn.classList.contains('custom-temp-hide')){
                                btn.classList.replace('custom-temp-hide', 'custom-temp-show');
                            }
                        });
                    }
            ...
            document.querySelectorAll(options.hideMoreFilterCls).forEach(btn => {
                btn.addEventListener("click", event => {
                    event.preventDefault();

                    if (document.querySelector(options.hideMoreFilterCls).classList.contains('more-filter-show')) {
                        document.querySelector(options.hideMoreFilterCls).classList.replace('more-filter-show', 'more-filter-hide');
                        document.querySelector(options.moreFilterCls).classList.replace('more-filter-hide', 'more-filter-show');

                        document.querySelectorAll('.custom-desktop-filter-item').forEach(btn => {
                            if(btn.classList.contains('custom-temp-show')){
                                btn.classList.replace('custom-temp-show', 'custom-temp-hide');
                            }
                        });
                    }
