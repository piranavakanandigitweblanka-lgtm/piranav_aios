...
class CartItems extends HTMLElement {
    ...
    initAutoRemove() {
      ...
      items.forEach(item => {
          if (item.dataset.isUnlisted === 'true' && item.dataset.isService !== 'true') {
              hasUnlisted = true;
          }
          if (item.dataset.isService === 'true') {
              serviceItem = item;
          }
      });

      if (!hasUnlisted && serviceItem) {
          this.updateQuantity(serviceItem.dataset.index, serviceItem.dataset.key, 0);
      }
    }
