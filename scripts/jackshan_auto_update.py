"""
jackshan_auto_update.py
Jackshan Dashboard Auto-Update — Requirement 1 & 2
Runs daily via Windows Task Scheduler.

What it does:
  1. Calculates rolling date windows (30-day + 7-day, ending yesterday)
  2. Queries Google Search Console API for all 50 products (live page totals + priority keyword)
  3. Queries Shopify Admin GraphQL API for monthly & weekly sales per product
  4. Applies Req 1 action rules and Req 2 optimization rules
  5. Rebuilds Staff-requirements/pages/jakshan.html in-place
  6. Writes a dated log to scripts/logs/
"""
...
# ── Paths ────────────────────────────────────────────────────────────────────
BASE_DIR    = r'C:\Users\PC\Documents\piranav_aios'
HTML_PATH   = os.path.join(BASE_DIR, r'Staff-requirements\pages\jakshan.html')
GSC_KEY     = r'C:\Users\PC\.keys\ga4-service-account.json'
SHOPIFY_TOK = open(r'C:\Users\PC\.keys\shopify-ledsone-token.txt').read().strip()
...
START_DATE = str(yesterday - timedelta(days=29))   # 30-day window
WEEK_START = str(yesterday - timedelta(days=6))    # 7-day window
...
creds    = Credentials.from_service_account_file(GSC_KEY, scopes=[SCOPE])
...
META = {
    ...
    'orange-painted-metal-shade-lighting-vintage-pendant-light': ('Retro Orange Pendant Light - Vintage Metal Ceiling Hanging Lamp UK', 'Retro orange pendant light with vintage metal shade. Perfect for kitchens, dining rooms & bold interior styles. Stylish hanging ceiling light for modern and retro décor.', 'Orange Painted Metal Shade Lighting Vintage Pendant Light~1507'),
...
# All live Shopify handles for the 50 products
LIVE_HANDLES = {h: (url or BASE_URL + h) for h, url in PRODUCTS}
# Map from live URL to dashboard handle
URL_TO_HANDLE = {(url or BASE_URL + h): h for h, url in PRODUCTS}
ALL_LIVE_HANDLES = set()
for h, url in PRODUCTS:
    live_h = (url or BASE_URL + h).replace(BASE_URL, '')
    ALL_LIVE_HANDLES.add(live_h)

# Special handle overrides (dashboard handle → live Shopify handle)
HANDLE_OVERRIDE = {
    'ledsone-industrial-vintage-32cm-orange-pendant-retro-metal-lamp-shade-e27-uk-holder':
...
def shopify_order_count(start, end):
    """Return {live_handle: order_count} for the given date range."""
    counts = {}
    cursor = None
    page = 0
    while True:
...
# ── STEP 2: GSC API — fetch data for all 50 products ─────────────────────────
...
# Read the existing HTML as a template (keep all CSS/JS structure, replace only data + stamps)
with open(HTML_PATH, encoding='utf-8') as f:
    html = f.read()

# Replace RAW_DATA block
html = re.sub(r'const RAW_DATA = \[.*?\];', r1_data_js, html, flags=re.DOTALL)
# Replace REQ2_DATA block
html = re.sub(r'const REQ2_DATA = \[.*?\];', r2_data_js, html, flags=re.DOTALL)

# Update generated timestamp in header
html = re.sub(r'Generated: \d{4}-\d{2}-\d{2}', f'Generated: {date.today()}', html)
html = re.sub(r'Auto-updated: .*?\|.*?(?=<)', generated_stamp, html)

# Update date range labels in info-bar
html = re.sub(r'(<span>)(2026-\d{2}-\d{2})(</span>\n.*?Start Date)', rf'\g<1>{START_DATE}\g<3>', html)
html = re.sub(r'(<span>)(2026-\d{2}-\d{2})(</span>\n.*?End Date)',   rf'\g<1>{END_DATE}\g<3>', html)
...
# Update Req2 static KPI values (avg ctr, total clicks, total imp) in kpi-grid
...
# Update period labels in info-bar
html = re.sub(r'(<span>)2026-06-11(</span>)', rf'\g<1>{START_DATE}\g<2>', html)
html = re.sub(r'(<span>)2026-07-10(</span>)', rf'\g<1>{END_DATE}\g<2>', html)
